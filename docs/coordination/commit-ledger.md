...
| 6 | (este) | **correções da rodada 1 do concílio** — 11 achados fechados, 10 testes novos | build verde: 45 testes, 0 falhas; `assembleDebug` ok. Auto-crítica: o BLOCKER foi fechado **mudando a semântica** (claim preso vira `FAILED` em vez de voltar à fila) — é a escolha conservadora, e custa uma mensagem não enviada quando o processo morre antes do envio. Registrado como decisão, não como acerto óbvio | rodada 2 |
