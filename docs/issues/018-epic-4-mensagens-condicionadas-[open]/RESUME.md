...
- date: 2026-09-01
- status: `[open]` — primeiro corte **implementado e testado**, rodada encerrada em
  2026-09-01 09:35 no teto de 2 rodadas de concílio. Branch
  `feature/epic-4-scheduled-messages` (worktree `../WA-Keeper--epic4-scheduled-msg`):
  8 commits, **47 testes JVM, 0 falhas**. Sem PR, sem merge, sem push.
  Nada verificado em aparelho. **9 achados vivos** aguardam decisão do operador (item 7).

## Next Step (DO THIS FIRST) — decisão do operador

1. Decidir sobre os **9 achados vivos** do item 7 desta página. Há recomendação escrita
   para cada um, **nenhuma executada**; terceira rodada de concílio é proibida.
2. Responder no aparelho as perguntas dos itens 1, 2 e 3 — nada aqui foi exercitado
   em dispositivo.
3. Decidir a questão de produto do item 4 (duas mensagens armadas para a mesma conversa).
...
5. **A branch `feature/epic-3-image-retention` está atrás de `development`.**
   *(Corrigido pelo coordenador: esta frente relatou que a branch "saiu de `main`, não de
   `development`". O `merge-base` é `fe713e0`, que por acaso é a ponta atual de `main` —
   daí a impressão. Mas `fe713e0` **é ancestral de `development`**: não há história
   divergente e o merge é normal, não há base a "corrigir".)*
   O que é verdade e importa: a branch está **19 commits atrás de `development`** (18
   quando esta frente escreveu). Consequência prática: um `git diff` da epic-3 contra
   `development` **mente** — parece apagar `CallDetector`, `Beeper`, `VoiceCommandEngine`
   e o cache de ações de resposta, que na verdade só não existem naquela base. Um
   `--ours`/`--theirs` apressado no merge apaga os 19 commits de comandos de voz; o
   conflito de imports em `NotifListenerService.kt` precisa dos **dois** lados.
